...
DIST = {
    "log10_M": [FixedLimitsPowerLaw([6, 10]), FixedLimitsPowerLaw([6, 10])],
    "log10_mu": [FixedLimitsPowerLaw([1, 2]), FixedLimitsPowerLaw([1, 2])],
    "a" : [FixedLimitsTruncatedGaussian([0.1, 0.7]), FixedLimitsTruncatedGaussian([0.1, 0.7])],
...
def load_samples(path):
    with h5py.File(f'{path}/result.hdf5', 'r') as hf:
        ...
        samples = [
            ...
            for key in ['A_lambda_M', 'A_lambda_mu', 'A_mu_a', 'A_sigma_a', 'B_lambda_M', 'B_lambda_mu', 'B_mu_a', 'B_sigma_a', 'weight']
        ]
    return np.vstack(samples).T

def make_true_x(samples):
    return {
            "log10_M": {"lam_A": samples[0], "lam_B": samples[4]},
            "log10_mu": {"lam_A": samples[1], "lam_B": samples[5]},
            "a": {"mu_A": samples[2], "sigma_A": samples[3], "mu_B": samples[6], "sigma_B": samples[7]},
            "e0": {}
        }
...
base_path = '/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/_pop_MIX_B_B_NO_SF'
...
post_bottom_axes = [fig.add_subplot(gs_post_bottom[0, k]) for k in range(3)]
...
labels_post = ["$\\lambda^{\\mathrm{A}}_M$", "$\\lambda^{\\mathrm{A}}_{\\mu}$", "$\\mu^{\\mathrm{A}}_a$","$\\sigma^{\\mathrm{A}}_a$","$\\lambda^{\\mathrm{B}}_M$", "$\\lambda^{\\mathrm{B}}_{\\mu}$", "$\\mu^{\\mathrm{B}}_a$", "$\\sigma^{\\mathrm{B}}_a$", "$w$"]

y_positions = [0.95, 0.75, 0.55]
x_positions = [0.90, 0.05, 0.5, 0.90, 0.90, 0.05, 0.5, 0.90, 0.05]
ha_positions = ['right', 'left','center','right','right','left', 'center','right','left']
...
plt.savefig("/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/combine_plot_MIX_B_B_posterior1D_PPD.png", dpi=300, bbox_inches="tight")
plt.savefig("/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/combine_plot_MIX_B_B_posterior1D_PPD.pdf", dpi=300, bbox_inches="tight")