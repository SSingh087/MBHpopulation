...
# right side: 3x2 posterior panel
...
post_axes = np.array([[fig.add_subplot(gs_right[r, c]) for c in range(2)] for r in range(3)])
...
labels_post = ["$x_c$", "$\\gamma_{\\mu}$", "$\\alpha_a$", "$\\beta_a$", "$\\alpha_{e0}$", "$\\beta_{e0}$"]
y_positions = [0.95, 0.75, 0.55]
x_positions = [0.90, 0.90, 0.90, 0.05, 0.05, 0.05]
ha_positions = ['right','right','right','left','left','left']
...
plt.savefig("/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/combine_plot_A_posterior1D_PPD.png", dpi=300, bbox_inches="tight")
plt.savefig("/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/combine_plot_A_posterior1D_PPD.pdf", dpi=300, bbox_inches="tight")