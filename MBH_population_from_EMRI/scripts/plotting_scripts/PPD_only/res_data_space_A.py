...
DIST = {
    "log10_M": FixedLimitSchechterFunction([6, 10]),
    "log10_mu": FixedLimitsTruncatedSkewNormal([1, 2]),
    "a" : FixedLimitTruncatedBetaDistribution([0.1, 0.7]),
    "e0" : FixedLimitTruncatedBetaDistribution([0.1, 0.7]),
}
...
labels = ["$x_c$", "$\\alpha_{\\mu}$", "$\\alpha_a$", "$\\beta_a$", "$\\alpha_{e0}$", "$\\beta_{e0}$"]
...
def load_samples(path):
    with h5py.File(f'{path}/result.hdf5', 'r') as hf:
        samples = [
            ...
            for key in ['xc_M', 'alpha_mu', 'alpha_a', 'beta_a', 'alpha_e0', 'beta_e0']
        ]
...
base_path = '/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/_pop_A_NO_SF'
...
catalogue_true = popdist.draw_samples(true_x_A, weight=1.0, size=SIZE)
...
for total in range(len(samples_all)):
    ...
    ax[1,0].plot(a_centers, percentiles_a[1], color=colors[total], label=INJ[total])
...
ax[0,1].hist(catalogue_true['log10_mu'], color='black', lw=2, label='True', histtype='step', bins=bins)
ax[1,0].hist(catalogue_true['a'], color='black', lw=2, label='True', histtype='step', bins=bins)
ax[1,1].hist(catalogue_true['e0'], color='black', lw=2, label='True', histtype='step', bins=bins)
...
plt.tight_layout()
plt.savefig('/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/PPD_A_hist.png', dpi=300)
plt.savefig('/data/wiay/postgrads/shashwat/EMRI_data/INFERENCE_DATA/PLOTS/PPD_A_hist.pdf', dpi=300)